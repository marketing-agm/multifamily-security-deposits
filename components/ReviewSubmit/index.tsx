...
export function ReviewSubmit({ returnId }: Props) {
  ...
  // Only non-zero charge rows show in the receipt
  const chargeRows: { label: string; amount: number }[] = [];
  if (calculatedCharges.rentDue > 0)
    chargeRows.push({ label: tenantData.leaseBreak ? 'Rent due (lease break)' : 'Rent due', amount: calculatedCharges.rentDue });
  if (calculatedCharges.utilityCharge > 0)
    chargeRows.push({ label: utilityData.utilityType === 'RUBS' ? 'RUBS chargeback' : 'Utility', amount: calculatedCharges.utilityCharge });
  ...
  if (manualCharges.other1 > 0)
    chargeRows.push({ label: manualCharges.other1Label || 'Other', amount: manualCharges.other1 });
  if (manualCharges.other2 > 0)
    chargeRows.push({ label: manualCharges.other2Label || 'Other (2)', amount: manualCharges.other2 });
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f3]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-5">
        ...
        {/* Deadline notice — days remaining, color-coded */}
        <DeadlineBanner moveOutDate={tenantData.moveOutDate} />

        {/* Two equal columns */}
        <div className="grid grid-cols-2 gap-5 items-start">

          {/* ══ LEFT: Receipt-style calculation summary ══ */}
          <div className="bg-white border border-[#e8e7e4] rounded-[8px] p-5 space-y-4">
            ...
            <div className="bg-[#f7f6f3] rounded-[6px] p-4 space-y-2">
              ...
              {chargeRows.length === 0 && (
                <p className="text-[12px] text-[#9b9b99]">No charges entered.</p>
              )}
              {chargeRows.map(row => (
            ...
            {/* Single compliance checkbox — must check before download unlocks */}
          ...
          <div className="bg-white border border-[#e8e7e4] rounded-[8px] overflow-hidden">
            ...
            <div className="px-5 pt-4 pb-2">
              ...
              <div className="space-y-0">
                {([
                  ...
                  ['Utility type', utilityData.utilityType === 'RUBS' ? `RUBS · ${formatCurrency(calculatedCharges.utilityCharge)} applied` : 'Flat fee'],
                  ['Lease break', tenantData.leaseBreak ? `Yes — new tenant ${tenantData.newTenantMoveInDate || tenantData.leaseEndDate}` : 'No'],
                  ...
                  <div key={label} className="flex justify-between text-[11.5px] py-1.5 border-b border-[#f0f0ee] last:border-b-0">
            ...
            <div className="px-5 pb-5 pt-3">
              <button
                onClick={handleDownload}
                disabled={!complianceChecked || generating}
                className="w-full py-2.5 text-[13px] font-medium rounded-[6px] transition-colors disabled:cursor-not-allowed"
                style={{
